...
# 数据库迁移（首次启动或 schema 变更后，entrypoint.sh 会自动执行）
...
1. `GET /api/v1/search?q=...` 使用 `plainto_tsquery('simple', ...)` 对 `Document.search_vector` 进行全文检索，并以 `ts_rank` 计算相关度。
...
| `20250630_004_seed_categories.py` | 插入初始分类数据（技术、产品、运营、后端、前端），解决前端上传页面分类下拉为空的问题。 |
...
`tests/` 下的测试使用 `pytest` + `TestClient`，搭配**内存 SQLite** 数据库和 `StaticPool`。通过 `app.dependency_overrides[get_session]` 注入 SQLite 会话。测试无需 Docker 即可快速运行，但**无法测试 PostgreSQL 特有功能**（`TSVECTOR`、触发器、GIN 索引）。PostgreSQL 相关行为请使用 `docker-compose exec web pytest ...` 验证。
...
- `command` 统一使用数组格式（避免 Docker 的 `sh -c` 包装），解决 PID 1 信号转发和 `$1` 参数解析问题。
- `redis` 和 `db` 均配置 `healthcheck`，所有上游服务通过 `condition: service_healthy` 等待依赖就绪。
- `web` 和 `worker` 设置 `stop_grace_period: 30s`，保证优雅关闭。
- `worker` 和 `beat` 添加 `PYTHONPATH=/app`，确保 Celery 能正确找到应用模块。
- `docker/entrypoint.sh` 为 web 容器提供自动迁移和数据库就绪检测。
- **Alembic** 迁移位于 `alembic/versions/`。容器启动时 `entrypoint.sh` 自动检测 web 服务并执行 `alembic upgrade head`（idempotent，跳过 celery worker/beat），无需手动操作。
...
- 注册接口使用 `application/x-www-form-urlencoded`（`URLSearchParams`），匹配后端 `Form(...)` 参数。
- 全局错误拦截器统一处理 `detail` 字段（数组/对象/字符串），避免显示 `[object Object]`。
- Element Plus 表单验证失败通过 `catch` 拦截，防止未捕获的 Promise 拒绝阻断上传流程。
...
- `app/services/search_service.py` — 全文检索、高亮、建议、热搜。
- `app/services/spell_corrector.py` — 基于标题和历史查询词的拼写纠错。
- `app/routers/documents.py` — 文档上传（异步）、状态轮询、原子浏览量计数。
- `frontend/src/utils/request.js` — Axios 实例，含全局错误拦截器（统一处理 `detail` 数组/对象）。
- `frontend/src/views/UploadView.vue` — 文档上传页面，含分类级联选择、表单验证、解析状态轮询。
- `docker/entrypoint.sh` — 容器入口脚本，自动数据库就绪检测 + `alembic upgrade head`。
- `alembic/versions/` — 数据库迁移。**003** 为性能优化迁移，**004** 为种子数据迁移。
...
启动第二阶段时，创建 `alembic/versions/2025xxxxx_005_phase2.py` 存放新表，详细需求参考 `PRD.md`。
