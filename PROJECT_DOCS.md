...
### 6.3 第二阶段：功能完善（W7-W10）

| 周次 | 模块 | 核心任务 |
|------|------|----------|
| W7 | 版本历史 | `DocumentVersion` 模型、diff 对比、版本回滚（保留最近 20 版） |
| W8 | 附件管理 | `Attachment` 模型、上传/下载、存储抽象层（Local/MinIO 预留） |
| W9 | 个人知识库 | `UserFavorite` / `UserCollection` 模型、收藏/个人分类 |
| W10 | 贡献统计 | ECharts 看板、作者排行、文档热度、更新频率 |

**技术决策**：
- 版本存储：完整内容存储（非增量），回滚简单，20 版 × 1万文档 = 20万行，PostgreSQL 可承载
- diff 算法：Python `difflib.SequenceMatcher`
- 存储抽象：预留 `StorageBackend` 接口，`LocalStorage` 和 `MinIOStorage`

### 6.4 第三阶段：体验优化与生产强化（W11-W14）

| 周次 | 模块 | 核心任务 |
|------|------|----------|
| W11 | 相关推荐 | 基于标签 Jaccard + 分类邻近度 + 作者相同的相关文档推荐 |
| W12 | 搜索升级 | 数据量 > 50万或相关性不足时评估 Elasticsearch 迁移；否则 PG 深度优化 |
| W13 | 监控告警 | Prometheus + Grafana + 钉钉/企微 Webhook 告警；结构化日志 + trace_id |
| W14 | SSO 接入 | `AuthProvider` 抽象层；LDAP / OAuth2 企业微信/钉钉/飞书登录；HTTPS 强制 |
...
| 债务 | 产生阶段 | 偿还计划 | 影响 |
|------|----------|----------|------|
| /tmp 文件存储 | MVP | W8 附件管理时迁移到持久目录 | 容器重启丢文件 |
| 无分片上传 | MVP | 第二阶段后期评估 | 大附件上传体验差 |
| 无全文检索集群 | MVP | W12 评估 ES 迁移 | 50万+ 文档时性能下降 |
| 简单 RBAC | MVP | 第三阶段权限映射预留 | 复杂权限场景不满足 |
...
| 第二阶段（W7-W10） | 2人 | 1人 | 兼职 | 2核4G（附件量大/并发增加） |
| 第三阶段（W11-W14） | 2人 | 1人 | 0.5人 | 2核4G（监控 + ES 评估需额外资源） |
